...
function normPath(p?: string | null) {
  if (!p) return "/";
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-pitch-border bg-pitch-black">
      ...
      <div className="flex items-center gap-1 px-4 py-3">
        {MENU.map((menu) => {
          const firstHref = menu.items?.[0]?.href; // <= target link pertama
          const isActive = currentActiveMenu === menu.key;
          const disabled = !firstHref;
          ...
          return (
            ...
            >
              {/* Ganti button -> Link agar klik = klik link pertama */}
              <Link
                href={firstHref || "#"}
                prefetch
